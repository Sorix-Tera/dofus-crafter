...
export default function App() {
  ...
  async function pickItem(it) {
    ...
    try {
      ...
      console.error(e)
      alert('Impossible de charger le détail de l\'objet (seuls les équipements sont supportés dans ce MVP).')
    }
  }

  const saveWatch = async () => {
    if (!picked) return
    const imageFromDetail = itemDetail?.image_urls?.thumb || itemDetail?.image_urls?.hq
    const imageFromPick = picked.image_urls?.thumb || picked.image_urls?.hq
    const w = {
      ankama_id: picked.ankama_id,
      name: picked.name,
      level: picked.level,
      imageUrl: imageFromDetail || imageFromPick || null,
  ...
  return (
    <div className="container">
      ...
      {picked && itemDetail && (
        <div className="card" style={{marginTop:16}}>
          <div className="flex" style={{justifyContent:'space-between'}}>
            <div className="flex">
              {(itemDetail?.image_urls?.thumb || picked?.image_urls?.thumb) && <img className="item" src={itemDetail?.image_urls?.thumb || picked?.image_urls?.thumb} />}
              <div>
      ...
      {!picked && (
        <div className="card" style={{marginTop:16}}>
          <div className="small">Cherche un équipement pour afficher sa recette et calculer le profit.</div>
        </div>
      )}
    </div>
  )
